...
<template>
  <div class="reference-images space-y-3.5" data-testid="reference-images" aria-label="参考图和网页图片">
    <h3 class="text-sm font-semibold text-stone-700">参考图</h3>

    <div v-if="store.hasMixedRefSources" class="flex items-center gap-2 p-2 rounded-lg border border-amber-200 bg-amber-50/90 text-amber-800 text-xs" data-testid="mix-conflict-warning">
      <AlertTriangle :size="14" class="shrink-0" />
      <span>不能同时使用本地上传和网页图片 URL，请选择其中一种</span>
    </div>

    <div v-if="validationError" class="p-2 rounded-lg border border-red-200 bg-red-50/90 text-red-700 text-xs" data-testid="validation-error">
    ...
    <div>
      ...
      <button
        ...
        class="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg border-2 border-dashed border-stone-300 text-sm font-medium text-stone-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        type="button"
    ...
    <div v-if="store.webImageUrls.length > 0" class="space-y-1.5" data-testid="web-url-list">
      <div
        ...
        :key="index"
        class="flex items-center gap-1.5 text-xs text-stone-600 bg-stone-50/90 rounded-lg border border-stone-200 px-2 py-1.5"
      >
