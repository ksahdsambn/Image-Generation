...
<template>
  <div class="result-grid" data-testid="result-grid" aria-label="生成结果">
    ...
    <div v-else-if="generationStore.isGenerating" class="flex min-h-[300px] flex-col items-center justify-center py-12 text-stone-500" data-testid="loading-state">
      ...
      <p class="text-sm font-medium">图片生成中，请稍候...</p>
    </div>
    ...
    <div v-else-if="!generationStore.hasResults" class="flex min-h-[300px] flex-col items-center justify-center py-12 text-stone-500" data-testid="empty-state">
      ...
      <p class="text-sm font-semibold text-stone-700">暂无生成结果</p>
      <p class="text-xs mt-1 text-stone-500">输入 Prompt 并点击生成按钮开始</p>
    </div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5" data-testid="result-images">
        <div
        ...
        >
          ...
          />
          <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-stone-950/72 to-transparent p-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
